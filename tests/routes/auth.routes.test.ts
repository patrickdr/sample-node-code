...
const makeApp = (authServiceOverrides: Partial<Record<keyof AuthService, jest.Mock>> = {}) => {
  const authService = {
...
describe('POST /api/auth/login', () => {
  ...
  it('returns 401 when credentials are invalid', async () => {
    const app = makeApp({ login: jest.fn().mockRejectedValue(new UnauthorizedError('Invalid credentials')) });
